...
function (_React$Component) {
  ...
  function LoanForm(props) {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(LoanForm).call(this, props)); // bind handlers/methods to React Component
  ...
  _createClass(LoanForm, [{
    ...
    value: function handleSubmit(evt) {
      ...
      if (!this.state.currentLoan) {
        this.state.currentLoan = new Loan(); // TODO: call the external submit listener

        this.props.submitListener(this.state.currentLoan);
      }

      this.state.currentLoan.set('title', this.state.title);
      this.state.currentLoan.set('principal', Number(this.state.principal));
      this.state.currentLoan.set('term', Number(this.state.term));
      this.state.currentLoan.set('rate', Number(this.state.rate / 100));
      this.setState({
    ...
    value: function handleChange(evt) {
      console.log("".concat(evt.target.id, " changed..."));
      var obj = {};
